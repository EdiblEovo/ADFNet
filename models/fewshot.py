...
from .resnet import res_encoder
...
class FewShotSeg(nn.Module):
    ...
    def __init__(self, in_channels=3, pretrained_path=None, cfg=None):
        ...
        self.config = cfg or {'align': False, 'depth_enable': False}

        # Encoder
        '''if not self.config['depth_enable']:
            self.encoder = nn.Sequential(OrderedDict([
                ('backbone', Encoder(in_channels, self.pretrained_path, self.config['depth_enable'])),]))
        else:'''
        if self.config['use_resnet']:
            self.encoder = res_encoder(self.pretrained_path, self.config['depth_enable'], self.config['att_enable'])
        else:
            self.encoder = Encoder(in_channels, self.pretrained_path, self.config['depth_enable'], self.config['att_enable'])


    def forward(self, supp_imgs, supp_dep, fore_mask, back_mask, qry_imgs, qry_dep):
        """
        Args:
        ...
        dep_concat = torch.cat([torch.cat(way, dim=0) for way in supp_dep]
                                + [torch.cat(qry_dep, dim=0),], dim=0)
        img_fts = self.encoder(imgs_concat, dep_concat)
