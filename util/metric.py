...
class Metric(object):
    ...
    def record(self, pred, target, labels=None, n_run=None):
        ...
        for j, label in enumerate(labels):
            ...
            # Get all locations
            idx = np.where(target)
            all_idx = set(zip(idx[0].tolist(), idx[1].tolist()))
            ...
            if target_idx_j:  # if ground-truth contains this class
                ...
                tn_arr[label] = len(set.intersection((all_idx - pred_idx_j), (all_idx - target_idx_j)))
    ...
    def get_mAcc(self, labels=None, n_run=None):
        """
        Compute mean Accuracy

        Args:
            labels:
                specify a subset of labels to compute mean Acc, default is using all classes
        """
        if labels is None:
            labels = self.labels
        ...
        if n_run is None:
            ...
            fp_sum = [np.nansum(np.vstack(self.fp_lst[run]), axis=0).take(labels)
                      for run in range(self.n_runs)]
            tn_sum = [np.nansum(np.vstack(self.tn_lst[run]), axis=0).take(labels)
                      for run in range(self.n_runs)]
            fn_sum = [np.nansum(np.vstack(self.fn_lst[run]), axis=0).take(labels)
                      for run in range(self.n_runs)]

            # Compute mean Acc classwisely
            # Average across n_runs, then average over classes
            mAcc_class = np.vstack([(tp_sum[run] + tn_sum[run]) / (tp_sum[run] + fp_sum[run] + tn_sum[run] + fn_sum[run])
                                    for run in range(self.n_runs)])
            mAcc = mAcc_class.mean(axis=1)

            return (mAcc_class.mean(axis=0), mAcc_class.std(axis=0),
                    mAcc.mean(axis=0), mAcc.std(axis=0))
        else:
            ...
            tn_sum = np.nansum(np.vstack(self.tn_lst[n_run]), axis=0).take(labels)
            fn_sum = np.nansum(np.vstack(self.fn_lst[n_run]), axis=0).take(labels)

            # Compute mean IoU classwisely and average over classes
            mAcc_class = (tp_sum + tn_sum) / (tp_sum + fp_sum + tn_sum + fn_sum)
            mAcc = mAcc_class.mean()

            return mAcc_class, mAcc
